...
describe('ToggleNav', () => {
  ...
  it('should render as a <div /> by default',() => {
    ...
    expect(wrapper.find('[data-test="ToggleNav"]').type()).toEqual('div');
  });

  it('should render as a custom component',() => {
    const wrapper = mount(<ToggleNav as="span" />);

    expect(wrapper.find('[data-test="ToggleNav"]').type()).toEqual('span');
  });

  it('should be able to pass a custom className',() => {
    const wrapper = mount(<ToggleNav className="foo" />);

    expect(wrapper.find('[data-test="ToggleNav"]').hasClass('foo')).toBe(true);
  });

  it('should be able to pass arbitrary props',() => {
    const wrapper = mount(<ToggleNav data-foo="true" />);

    expect(wrapper.find('[data-test="ToggleNav"][data-foo="true"]')).toHaveLength(1)
  });
})
